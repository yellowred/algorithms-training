...
func findLadders1(beginWord string, endWord string, wordList []string) [][]string {
	...
	for len(queue) > 0 {
		current := queue[0]
		log.Println(current, level)
		counter++
		if level <= len(current)+1 && len(ans) > 0 {
			log.Println("done")
			break
		}
		...
		if lastWord == endWord {
			...
			for i := 0; i < len(lastWord); i++ {
				newWord := lastWord
				for c := 'a'; c <= 'z'; c++ {
					newWord = string(newWord[0:i] + string(c) + newWord[i+1:len(newWord)])
					if wl[newWord] && !visited[newWord] {
						newPath := make([]string, len(current)+1)
						copy(newPath, append(current, newWord))

						visited[newWord] = true

						if newWord == endWord {
							log.Println("NP=", newPath)
							ans = append(ans, make([]string, len(newPath)))
							copy(ans[len(ans)-1], newPath)
							level = len(newPath)
						} else {
							queue = append(queue, make([]string, len(newPath)))
							copy(queue[len(queue)-1], newPath)
						}
					}
				}
			}
...
func findLadders2(beginWord string, endWord string, wordList []string) [][]string {
	...
	ans := [][]string{}
	queue_s := list.New()
	visited_s := map[string]bool{}
	parent_s := map[string]string{}

	queue_d := list.New()
	visited_d := map[string]bool{}
	parent_d := map[string]string{}

	queue_s.PushBack(beginWord)
	queue_d.PushBack(endWord)
	...
	for queue_s.Len() > 0 && queue_d.Len() > 0 {
		bfs(queue_s, &visited_s, &parent_s, wl)
		bfs(queue_d, &visited_d, &parent_d, wl)
		log.Println("S", visited_s, len(visited_s))
		log.Println("D", visited_d, len(visited_d))
		log.Println("N", intersect(visited_s, visited_d))
		if inter := intersect(visited_s, visited_d); len(inter) > 0 {
			for _, k := range inter {
				sol := printPath(k, parent_s, parent_d, beginWord, endWord)
				if !arrayExists(sol, ans) {
					ans = append(ans, sol)
				}
...
func bfs(queue *list.List, visited *map[string]bool, parent *map[string]string, wl map[string]bool) {
	elem := queue.Front()
...
func intersect(visited_s, visited_d map[string]bool) []string {
	inter := []string{}
	for k := range visited_s {
		if visited_d[k] {
...
func printPath(k string, parent_s, parent_d map[string]string, beginWord, endWord string) []string {
	path_s := []string{}
	current := k
	for current != beginWord {
		current = parent_s[current]
		path_s = append(path_s, current)
	}
	ans := []string{}
	i := len(path_s) - 1
	for i >= 0 {
		ans = append(ans, path_s[i])
		i--
	}
	ans = append(ans, k)
	current = k
	for current != endWord {
		current = parent_d[current]
		ans = append(ans, current)
	}
	return ans
}

func arrayExists(a []string, b [][]string) bool {
	for _, elem := range b {
		if strings.Join(elem, "_") == strings.Join(a, "_") {
			return true
		}
	}
	return false

...
func findLadders2(beginWord string, endWord string, wordList []string) [][]string {
	...
	queue_s = append(queue_s, beginWord)
	queue_d = append(queue_d, endWord)

	for len(queue_s) > 0 && len(queue_d) > 0 {
		bfs(&queue_s, &visited_s, &parent_s)
		bfs(&queue_d, &visited_d, &parent_d)
		log.Println("S", visited_s, len(visited_s))
		log.Println("D", visited_d, len(visited_d))
		log.Println("N", intersect(visited_s, visited_d))
		if k := intersect(visited_s, visited_d); k != nil {
			ans = printPath(*k, parent_s, parent_d, beginWord, endWord)
			break
		}
	}
	return ans
...
func bfs(queue *[]string, visited *map[string]bool, parent *map[string]string) {
	current := (*queue)[0]

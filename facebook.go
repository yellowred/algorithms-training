...
func findLadders2(beginWord string, endWord string, wordList []string) [][]string {
	...
	paths = append(paths, []string{beginWord})
	level := 1
	...
	for len(paths) > 0 {
		...
		for i := 0; i < len(last); i++ {
			news := last
			for c := 'a'; c <= 'z'; c++ {
				news = string(news[0:i] + string(c) + news[i+1:len(news)])
				if wl[news] {
					...
					if news == endWord {
						...
						paths = append(paths, make([]string, len(newpath)))
						copy(paths[len(paths)-1], newpath)
					}
				}
			}
		}
